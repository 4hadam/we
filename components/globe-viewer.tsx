...
export default function GlobeViewer({
  ...
  useEffect(() => {
    ...
    const initGlobe = async () => {
      ...
      const globe = new (GlobeFactory as any)()(containerRef.current)
        .globeImageUrl(
      ...
      try {
        ...
        // --- بداية: تعديل اسم أمريكا ---
        const usaFeature = unifiedFeatures.find(
          (f: any) => f.properties.ADMIN === "United States of America"
        );
        if (usaFeature) {
          usaFeature.properties.ADMIN = "United States";
        }
        // --- نهاية: تعديل اسم أمريكا ---
        // 🔴🔴🔴 انتهى التعديل 🔴🔴🔴
    ...
    const init = async () => {
      const cleanupFn = await initGlobe();
      if (aborted) {
        cleanupFn?.();
        return;
      }
